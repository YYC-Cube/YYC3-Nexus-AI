...
import { Check, Pencil, RefreshCw, Square, X } from "lucide-react"
...
const ChatPane = forwardRef(function ChatPane(
  { conversation, onSend, onEditMessage, onResendMessage, isThinking, onPauseThinking }: ChatPaneProps,
  ref,
) {
  const { t } = useLocale()
  ...
  useImperativeHandle(
    ...
    () => ({
      insertTemplate: (templateContent: string) => {
        ;(composerRef.current as unknown as { insertTemplate: (c: string) => void })?.insertTemplate(templateContent)
      },
    }),
    [],
  )
                ...
                {editingId === m.id ? (
                  <div className={cls("rounded-2xl border p-2", "border-zinc-200 dark:border-zinc-800")}>
                    <textarea
                      ...
                      className="w-full resize-y rounded-xl bg-transparent p-2 text-sm outline-hidden"
                      rows={3}
                    />
